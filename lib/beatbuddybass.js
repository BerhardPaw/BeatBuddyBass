...
module.exports = {
	...
	checkDirPath: function (dirPath) {
		try {
			var stats = fs.lstatSync(path.normalize(dirPath));
			if (stats.isDirectory()) { return dirPath; }
		}
		catch (e) { return null; }
	},	
	//
	//
	getFullDirPath: function (dirPath, prefix) {
		var path_1 = path.normalize(dirPath);
		var path_2 = path.normalize(this.dirname + (_.isUndefined(prefix) ? "" : prefix + "/") + dirPath);
		var path_3 = path.normalize((_.isUndefined(prefix) ? "" : prefix + "/") + dirPath);

        var res = this.checkDirPath(path_1);
		if (!_.isEmpty(res)) {
			return res;
		}
		res = this.checkDirPath(path_2);
		if (!_.isEmpty(res)) {
			return res;
		}
		res = this.checkDirPath(path_3);
		if (!_.isEmpty(res)) {
			return res;
		}
		else {
			throw Error("Cannot locate File:" + dirPath);
		}
	},
	//
	//
	...
	writeSongPart: function (marker, song, drumEvents, bassEvents, fileName, opts) {
		if (marker.subtype == "marker" && marker.text != "") {
            ...
            var jsonPath = path.normalize(this.getFullDirPath(cf.config.directory.jsonPath) + "/" + markerFileName + ".json");
			fs.writeFileSync(jsonPath, JSON.stringify(jsonObj));

			var midiPath = path.normalize(this.getFullDirPath(cf.config.directory.outputPath) + "/" + markerFileName + ".mid");
