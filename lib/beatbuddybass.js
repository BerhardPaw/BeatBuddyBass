...
module.exports = {
	...
	//
	//
	writeSongPart : function (marker, song, drumEvents, bassEvents, fileName ) {
		if ( marker.subtype == "marker" && marker.text != "" ) {
            ...
            var jsonPath = this.getFullFilePath(markerFileName + ".json", cf.config.directory.jsonPath);
			fs.writeFileSync( jsonPath , JSON.stringify(jsonObj));
			
			var midiPath = this.getFullFilePath(markerFileName + ".mid", cf.config.directory.outputPath);
		...
		for (var f = 0; f < opts.filePaths.length; f++) {
			...
			for (var i = lastMarker; i >= 0 ; i-- ) {
				...
				if ( markers[i].subtype == "marker" && markers[i].text !== "" ) {	
					...
					//
					this.writeSongPart(markers[i], song, drumEvents, bassEvents, fileName );
				}	 
			}	// end of read markers
